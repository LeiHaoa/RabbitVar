...
std::tuple<string, bool, vector<string> > VarDictLauncher::get_wgs_region(Configuration *conf){
  robin_hood::unordered_map<string, int> chr2length;
  vector<string> segraw;
  readChr(conf->bam.getBamX(), chr2length);
  for(auto &chrlen : chr2length){
    segraw.push_back(chrlen.first + conf->delimiter + 
                    "0" + conf->delimiter +
                    std::to_string(chrlen.second) + conf->delimiter);
  }
  //ampliconParameters, zeroBased, segraw
  return std::tuple<string, bool, vector<string> >("", conf->zeroBased, segraw);
}
/**
...
std::tuple<string, bool, vector<string> > VarDictLauncher::readBedFile(Configuration *conf){
  ...
  if(conf->bed == "" && conf->regionOfInterest == ""){ //no region specified, detect WGS region
    return get_wgs_region(conf);
  }
  vector<string> segraw;
