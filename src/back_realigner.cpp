    ...
    for (SortPositionDescription* tpl : tmp) {
        try {
			...
			*/
            ...
            int misp5;
            string misnt5;
			int mm3_size = findMM3(ref, p, sanpseq, mmm, sc3p, nm3, misp3, misnt3);
            int mm5_size = findMM5(ref, p + dellen + extra.length() - extrains.length() - 1, wupseq, mmm, sc5p, nm5, misp5, misnt5);
            //if (conf.y) {
        ...
        for (SortPositionDescription* tpl : tmp) {
             ...
             */
                ...
                if (!ins3.empty()) {
                    ...
                    mm3_size = findMM3(ref, p3 + 1, sanpseq, mmm, sc3p, nm3, misp3, misnt3);
                } else {
                    sanpseq = tn + joinRef(ref, position + extra.length() + 1 + compm.length() + newdel, sanend);
                    mm3_size = findMM3(ref, position + 1, sanpseq, mmm, sc3p, nm3, misp3, misnt3);
                }

                mm5_size = findMM5(ref, position + extra.length() + compm.length() + newdel, wupseq, mmm, sc5p, nm5, misp5, misnt5);
				// mismatches, mismatch positions, 5 or 3 ends
                //MismatchResult *findmm5 = findMM5(ref, position + extra.length() + compm.length() + newdel, wupseq,
				//								  mmm, sc5p, nm5, misp5, misnt5);
                /*
...
int VariationRealigner::findMM5(robin_hood::unordered_map<int, char> &ref,
											int position,
											string wupseq,
...
int VariationRealigner::findMM3(robin_hood::unordered_map<int, char> &ref,
											int p,
											string sanpseq,
	                                        vector<Mismatch*> &mismatches, vector<int>& sc3p, int& mn, int& misp, string& misnt3) {
