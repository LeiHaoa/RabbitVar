...
static std::string vartype_map[5]  = {"Insertion", "Deletion", "Complex", "SNV", ""};
static std::string varlabel_map[9] = {"Germline", "StrongLOH", "LikelyLOH", "StrongSomatic", "LikelySomatic", 
                              "AFDiff", "SampleSpecific", "FALSE", ""};

std::string get_varlabel_str(VarLabelSet vl){
  return varlabel_map[vl];
}
std::string get_vartype_str(VartypeSet vt){
  return vartype_map[vt];
}
/**
 ...
 */
bool Variant::isNoise(Configuration *conf)
{
  const double qual = this->meanQuality;
  if (
    ((qual < 4.5 || (qual < 12 && !this->hasAtLeast2DiffQualities)) && this->positionCoverage <= 3) || (qual < conf->goodq && this->frequency < 2 * conf->lofreq && this->positionCoverage <= 1))
  {
...
void Variant::adjComplex()
{
  ...
  int n = 0;
  while (refAllele.length() - n > 1 && varAllele.length() - n > 1 && refAllele[n] == varAllele[n])
  {
    n++;
  }
  if (n > 0)
  ...
  n = 1;
  while (refAllele.length() - n > 0 && varAllele.length() - n > 0 && vc_substr(refAllele, -n, 1) == vc_substr(varAllele, -n, 1))
  {
    n++;
  }
  if (n > 1)
...
void Variant::debugVariantsContentSimple(vector<string> &tmp, string n)
{
  tmp.emplace_back(debugVariantsContent(n));
}

void Variant::debugVariantsContentInsertion(vector<string> &tmp, string n)
{
  string sb = "I";
...
string Variant::debugVariantsContent(string n)
{
  string sb = n;
 ...
 * @return variant type
 */
VartypeSet Variant::varType()
{
  if (refallele.length() == 1 && varallele.length() == 1)
  {
    return SNV;
  }
  else if (refallele.length() == 0 || varallele.length() == 0)
  { // issue #19
    return COMPLEX;
  }
  ...
  else if (refallele.length() == 1 && varallele.length() > 1 && starts_with(varallele, refallele))
  {
    return INSERTION;
  }
  else if (refallele.length() > 1 && varallele.length() == 1 && starts_with(refallele, varallele))
  {
    return DELETION;
  }
  return COMPLEX;
 ...
 */
bool Variant::isGoodVar(Variant *referenceVar, VartypeSet type,
               set<string> *splice, const Configuration *conf)
{
  ...
  if (frequency < conf->freq || hicnt < conf->minr || meanPosition < conf->readPosFilter || meanQuality < conf->goodq)
  {
    return false;
  }
  ...
  if (referenceVar != NULL && referenceVar->hicnt > conf->minr && frequency < 0.25d)
  {
    ...
    if ((d - 2 < 5 && referenceVar->meanMappingQuality > 20) || f < 0.25)
    {
      return false;
    }
  }
  ...
  if (strandBiasFlag == 0x21 && frequency < 0.20)
  {
    if (type == INIT_EMPTY || type == SNV || (refallele.length() < 3 && varallele.length() < 3))
    {
      return false;
    }
  }
...
string Variant::tostring()
{
  ...
  str = str +
        ...
        ", varallele='" + varallele + "\'" +
        ", refallele='" + refallele + "\'" +
        ", vartype='" + get_vartype_str(vartype) + "\'" +
